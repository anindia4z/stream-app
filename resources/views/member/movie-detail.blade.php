...
@section('content')
    ...
    <div class="flex gap-14 items-start">
     ...
     {{--            <form action="{{ route('member.movie.like', ['movieId' => $movie->id]) }}" method="POST">
                    @csrf
                    @method('POST')
                    <button type="submit" class="flex items-center justify-center text-white font-semibold py-2 px-2 rounded-lg ms-2">
                        @if ($user->favoriteMovies->contains($movie))
                            <!-- Status disukai: Warna merah -->
                        ...
                        @else
                            <!-- Status tidak disukai: Warna putih -->
