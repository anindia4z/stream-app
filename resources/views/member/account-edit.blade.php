...
@section('content')
    <form action="{{ route('account.setting.update', $user->id) }}" method="POST" enctype="multipart/form-data" class="flex flex-col rounded-2xl gap-6">
        @csrf
        @method('PUT')
        <div class="rounded-2xl bg-[#19152E] p-5 w-3/4 mx-auto space-y-6">
...
<script>
    function previewImage(event) {
        ...
        reader.onload = function(e) {
            // Perbarui gambar di preview (bagian edit)
            const imagePreview = document.getElementById('imagePreview');
            imagePreview.src = e.target.result;
            
            // Jika perlu, juga perbarui gambar di navbar (jika Anda ingin preview langsung di navbar)
